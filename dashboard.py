...
with aba3:
    quant_vendedores = st.number_input('Quantidade de Vendedores', 2, 10, 5)
    col1, col2 = st.columns(2)
    with col1:
        st.metric("Receita", format_number(dados['Preço'].sum(), 'R$'))
        fig_receita_vendedores = px.bar(vendedores[['sum']].sort_values('sum', ascending=False).head(quant_vendedores),
                                        x='sum',
                                        y=vendedores[['sum']].sort_values('sum', ascending=False).head(quant_vendedores).index,
                                        text_auto=True,
                                        title=f'Top {quant_vendedores} vendedores (receita)')
        st.plotly_chart(fig_receita_vendedores, use_container_width=True)

    with col2:
        st.metric("Quantidade de vendas", format_number(dados.shape[0]))
        fig_vendas_vendedores = px.bar(vendedores[['count']].sort_values('count', ascending=False).head(quant_vendedores),
                                        x='count',
                                        y=vendedores[['count']].sort_values('count', ascending=False).head(quant_vendedores).index,
                                        text_auto=True,
